...
This artifact checks the VAD for executable sections that are not mapped to disk
and has suspicious content which may indicate process injection.

User options allow targeting process, modifying suspicious content yara, or
...
parameters:
  ...
  - name: ProtectionRegex
    description: |
        Protection of section. Default is Executable but can customize for other use cases.
