...
This artifact can be used to retrieve and parse some FreeFileSync file in order to:
- Identify the latest account used to transfer data and the remote ip adresse destination in the case of SFTP protocol with the query GlobalInfo
- Identify the latest transferred files with Latest Data Transfer
...
parameters:
   - name: FreeFileInfoGlob
     ...
     description: Use to retrieve latest global information including userID and remote destination for SFTP from GlobalSettings.xml file
   ...
   - name: FreeFileSyncGlob
     ...
     description: Use to retrieve and parse latest html log file
