...
During security incidents many environments are locked down and it is not
possible to the Velociraptor server to reach out to fetch tools from the
internet when needed (i.e. internet access is shut down or blocked). You might
sometimes also have to work in environments that are permanently air-gapped.

In these environments, we need to manually pre-populate the tools inventory with
the binaries of all the tools we would like to use in advance. This also gives
us the opportunity to curate the tools that will actually be allowed.
...
1. The first time you collect it, it will create a downloader bash script which
...
2. On an internet-connected system you create a folder and then run the script
   produced in the first step from that folder. This downloads all the binaries.
