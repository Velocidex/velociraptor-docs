...
| **name** | **description** |
|---|---|
| `Server.Audit.Logs` | Collects server audit events for significant user actions such as starting a new collection, creating a new hunt, updating an artifact definition etc. |
| `Server.Internal.Alerts` | Provides an internal event stream for high-value alerts generated by client or server artifacts. |
| `Server.Internal.ArtifactModification` | Provides an internal event stream for artifact modification notifications. |
| `Server.Internal.ClientDelete` | Provides an internal event stream for client deletion notifications. |
| `Server.Internal.MetadataModifications` | Emits an event when server metadata (such as client labels or host info) is modified. |
...
To send events to a server event queue you use the
