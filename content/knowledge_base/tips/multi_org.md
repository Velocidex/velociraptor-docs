...
Orgs are separated out so when you run a query you are running that query within the context of the org. Normally the hunts scheduled in an organization can be accessed using the [hunts()](/vql_reference/server/hunts/) plugin, but that normally acts within a single Org.

To run a query in another org, you can switch org contexts using the [query()](/vql_reference/server/query/) plugin.
...
Some plugins (e.g. [hunt()](/vql_reference/server/hunt/) ) support orgs directly for convenience but generally you should use the above approach. This will also remind you that each such query is running in a separate org context and therefore can not see other data at the same time.
